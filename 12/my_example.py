...
# 교차 검증 점수를 기반으로 최적의 하이퍼 파라메터를 검색할 수 있는 gridSearchCV 클래스
...
# (예측기 객체, 테스트 파라메터의 딕셔너리 객체, cv=교차검증 폴드 수,...)
...
# 모든 하이퍼 파라메터를 조합하여 평가한 가장 높은 교차검증 SCORE 값을 반환
print(f'best_score -> {grid_model.best_score_}')
# 가장 높은 교차검증 SCORE 가 어떤 하이퍼 파라메터를 조합했을 때 만들어 졌는지 확인
print(f'best_params -> {grid_model.best_params_}')
# 가장 높은 교차검증 SCORE 의 하이퍼 파라메터를 사용하여 생성된 모델 객체를 반
